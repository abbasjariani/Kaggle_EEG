...
n_layers_to_search = [1,2]
n_neurons_per_layer_to_search = [2,3]
n_processors = 3
#####################################
kfold_crossval = 5
epochs_nn = 200
...
log_f_path = '/home/abbas/projects/180710_EEG_DeepL/greedy_search_channels_NN_180803/' + cur_sub + '_greedy_NN_channel_search.log'
try:
...
X_array  = cur_subj_feat_data.values
X_array.shape
X_array = np.delete(X_array, 0, 1)  #two first columns are not data
X_array = np.delete(X_array, 0, 1)  
X_array_scaled = preprocessing.scale(X_array)    
y_train_inverse = np.empty(Y_array.shape)
y_train_inverse[Y_array == 0.] = 1.
y_train_inverse[Y_array == 1.] = 0.
y_train_binary = np.concatenate((Y_array, y_train_inverse),axis=1)
...
def get_structure_combinations(n_layers_to_search, n_neurons_per_layer_to_search):
    ...
    for cur_n_layer in n_layers_to_search:
        ...
        combinations_n_neuron_list.append(list(itertools.product(*comb_ls)))
